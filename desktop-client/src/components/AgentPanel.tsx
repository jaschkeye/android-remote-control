...
import { Send, Square, Bot, User, Loader2, CheckCircle2, AlertCircle, Hand, Settings } from 'lucide-react';
...
interface Message {
  ...
  content: string;
  type?: 'text' | 'action' | 'screenshot' | 'error' | 'done';
  detail?: string;
}
      ...
      switch (event.type) {
        ...
        case 'action':
          setMessages((m) => {
            ...
            if (last && last.type === 'action' && last.role === 'agent') {
              return [...m.slice(0, -1), {
                ...last,
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-[var(--text-tertiary)] text-xs">例如：打开微信发消息给张三</p>
          </div>
        )}
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center ${
              msg.role === 'user' ? 'bg-[var(--accent-dim)]' : msg.type === 'error' ? 'bg-red-900/40' : msg.type === 'done' ? 'bg-green-900/40' : 'bg-[var(--bg-tertiary)]'
            }`}>
              {msg.role === 'user' ? <User className="w-3.5 h-3.5 text-[var(--accent)]" /> :
               ...
               msg.type === 'action' ? <Hand className="w-3.5 h-3.5 text-[var(--accent)]" /> :
            ...
            <div className={`max-w-[80%] px-3 py-2 rounded-lg text-sm ${
              ...
              msg.type === 'action' ? 'bg-[var(--bg-tertiary)] text-[var(--text-primary)] border border-[var(--border)]' :
